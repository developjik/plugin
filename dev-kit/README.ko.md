...
복잡도는 여전히 중요하지만, 더 이상 사용자에게 보이는 파이프라인을 바꾸지는 않습니다. 대신 `clarify`가 작업 복잡도를 평가해 execution profile을 정하고, `planning`이 내부 `draft -> critique -> revise -> freeze` 게이트를 통해 plan 품질을 책임지며 execute readiness까지 닫습니다. 그 뒤 `execute`는 순수 실행 단계로 동작하고, `review`는 승인된 plan 기준으로 최종 결과를 검증합니다.
...
planning
  - plan.md 초안을 작성한다
  - 독립 critique를 수행한다
  - critique 결과를 plan-review.md에 기록한다
...
|---|---|---|
| **clarify** | "I want to...", "I need...", "let's build...", "can you help me..." 같은 모호한 요청 | Context Brief를 만들고, 복잡도를 평가하며, planning을 위한 `.dev-kit/` 세션 상태를 초기화합니다. |
| **planning** | clarify 완료 후, 또는 충분히 명확한 plan 요청 | `plan.md` 초안을 쓰고, 독립 critique를 수행하고, `plan-review.md`를 기록하고, execute readiness를 증명한 뒤 승인된 plan을 freeze하고 active session state를 실행 상태로 업데이트합니다. |
| **execute** | "run the plan", "execute the plan", "let's start implementing" | 승인된 plan만 실행하는 통합 실행 오케스트레이터입니다. worker-validator loop를 돌리고, phased run용 checkpoint JSON을 쓰고, 완료된 작업을 review로 넘깁니다. |
| **review** | "review the work", "verify the implementation", "check if the plan was executed correctly" | 승인된 plan에 대한 최종 독립 검증입니다. 마이그레이션 중 내부적으로는 `review-execute` skill이 구현합니다. |
...
- `in_progress` — 세션이 `clarify`, `planning`, `execute`, `review` 중 하나 안에서 진행 중인 상태
- `completed` — `review`가 통과한 뒤의 최종 성공 상태
...
- `in_review` — 독립 critique가 draft plan을 검토 중인 상태
- `revising` — critique 결과를 반영해 draft를 수정 중인 상태
...
**One Visible Flow** — 복잡도는 실행 강도에만 영향을 주고, 사용자에게 보이는 라우팅은 바꾸지 않습니다.

**Planning Owns Plan Quality** — `planning`은 `execute` 전에 plan을 draft, critique, revise, freeze까지 마쳐야 합니다.
...
**Review Verifies Results Only** — `review`는 최종 코드베이스를 승인된 plan과 비교하고, implementation drift가 있을 때만 `execute`로 돌려보냅니다.

**JSON Source Of Truth** — `state.json`이 canonical source이고, Markdown 문서는 사람이 읽는 파생물입니다.

**Shared Session Recovery** — planning, execute, review는 별도 `resume` skill 대신 공용 `.dev-kit/current.json` 포인터와 session scan helper를 사용합니다.
