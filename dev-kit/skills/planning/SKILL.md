---
name: planning
description: Use when a task's scope is clear and multi-step implementation is needed, before touching code. Triggered after clarify is complete, or when the user explicitly requests plan creation with a clear prompt.
---
...
- After the clarify skill completes and a Context Brief file has been generated
...
This skill operates within a session directory at `docs/sessions/<session-id>/`.

- If the user or resume skill provides a session path directly, use it.
- Otherwise, scan `docs/sessions/` for the most recent session where clarify is completed and planning is not yet completed.
- Read `state.md` to find the session state and `brief.md` as the Context Brief input.

This skill takes a **Context Brief file** as input. The Context Brief generated by the `clarify` skill is used to populate the plan header:
...
> **Worker note:** Execute this plan task-by-task using the execute skill or subagents. Each step uses checkbox (`- [ ]`) syntax for progress tracking.
...
**If the final verification fails**, the plan is not complete. The worker-validator loop in `execute` will handle failure response (see execute's E2E Failure Response Protocol).
...
**2. Inline execution** — execute tasks in this session using the execute skill, batch execution with checkpoints
...
- When the plan is ready to execute → `execute` skill
- If ambiguity is discovered in the work scope → return to the `clarify` skill to resolve
