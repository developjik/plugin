...
- After `execute-plan` completes and the implementation works but reads like AI wrote it
...
- If the code was generated by `execute-plan` → report results to the user
- If implementation discipline was lacking during generation → consider applying `write-surgically` in future sessions to prevent slop at the source
