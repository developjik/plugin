...
- After the `clarify-requirements` skill completes and a Context Brief file has been generated
...
This skill takes a **Context Brief file** as input. The Context Brief generated by the `clarify-requirements` skill is used to populate the plan header:
...
> **Worker note:** Execute this plan task-by-task using the execute-plan skill or subagents. Each step uses checkbox (`- [ ]`) syntax for progress tracking.
...
**If the final verification fails**, the plan is not complete. The worker-validator loop in `execute-plan` will handle failure response (see execute-plan's E2E Failure Response Protocol).
...
**"Plan complete and saved to `docs/dev-kit/plans/<filename>.md`."**

**"How would you like to proceed?"**

**1. Subagent execution (recommended)** — dispatch a fresh subagent per task, review between tasks, fast iteration

**2. Inline execution** — execute tasks in this session using the execute-plan skill, batch execution with checkpoints
...
- When the plan is ready to execute → `execute-plan` skill
- If ambiguity is discovered in the work scope → return to the `clarify-requirements` skill to resolve
