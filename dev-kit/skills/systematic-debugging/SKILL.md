...
- If debugging was invoked from a pipeline skill (execute, review) -> return to the invoking skill or check `.dev-kit/current.json` plus the active session `state.json` for the current next action
