...
STATE_STATUSES = {"in_progress", "completed"}
RESUMABLE_STATUSES = {"in_progress"}
PHASES = {"clarify", "planning", "execute", "review"}
...
def validate_state_schema(schema: Any) -> list[str]:
    ...
    if properties.get("status", {}).get("enum") != ["in_progress", "completed"]:
        errors.append("Bundled state schema status enum is invalid")
    if properties.get("current_phase", {}).get("enum") != ["clarify", "planning", "execute", "review"]:
...
def validate_state_payload(
    ...
    if status not in status_enum:
        errors.append("state.json status must be one of in_progress, completed")

    current_phase_enum = properties.get("current_phase", {}).get("enum", [])
    current_phase = payload.get("current_phase")
    if current_phase not in current_phase_enum:
        errors.append("state.json current_phase must be one of clarify, planning, execute, review")
    ...
    if current_phase == "review" and plan_status != "approved":
        errors.append("state.json current_phase review requires plan_status approved")

    if status == "completed":
        if current_phase != "review":
            errors.append("state.json status completed requires current_phase review")
...
def validate_materialized_artifacts(payload: Any, workspace_root: Path) -> list[str]:
    ...
    if current_phase in {"planning", "execute", "review"}:
