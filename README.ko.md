...
Codex에서 로컬 플러그인을 설치하는 방법은 별도 가이드를 참고하세요.

- [Codex 로컬 플러그인 설치 가이드](./docs/codex-local-plugin-install.ko.md)

Claude Code에서 로컬 플러그인을 설치하는 방법도 별도 가이드를 참고하세요.

- [Claude Code 로컬 플러그인 설치 가이드](./docs/claude-code-local-plugin-install.ko.md)

## 포함된 플러그인

| 플러그인 | 목적 | 문서 |
|---|---|---|
| `dev-kit` | `clarify -> planning -> execute -> review-execute` 흐름을 사용하는 구조화된 개발 워크플로우 플러그인 | [영문](./dev-kit/README.md) / [한글](./dev-kit/README.ko.md) |
